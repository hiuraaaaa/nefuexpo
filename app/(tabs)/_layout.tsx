...
function TabIcon({ focused, label, iconActive, iconInactive, badge }: {
  ...
  const pillStyle = useAnimatedStyle(() => ({
    ...
    backgroundColor: `${theme.accent}${Math.round(iconColor.value * 0x22).toString(16).padStart(2, '0')}`,
  }));
  ...
  return (
    <View style={styles.tabItem}>
      ...
      {/* Label — hanya muncul saat fokus */}
      <Text
        style={[
          styles.label,
          {
            color: focused ? theme.accent : 'transparent',
            fontSize: focused ? 9 : 0,
          },
        ]}
        numberOfLines={1}
      >
...
// ── Tab Bar Background — solid, no blur ────────────────────────────────────────
