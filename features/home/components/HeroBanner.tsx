...
export const calcHeroHeight = (w: number) => 224 + Math.max(0, (w - 380) * 0.15);
                ...
                >
                  ...
                  <View style={{ flex: 1, gap: 8, paddingBottom: 4 }}>
                    ...
                    <View style={{ flexDirection: 'row', alignItems: 'center', gap: 14, marginTop: 2 }}>
                      ...
                      {/* Moon-dot progress — inline di sebelah tombol, bukan absolute lagi */}
                      {items.length > 1 && (
                        <View style={{ flexDirection: 'row', gap: 5, alignItems: 'center' }}>
                          {items.slice(0, 8).map((_, dotI) => (
                            <TouchableOpacity key={dotI} onPress={() => goTo(dotI)} hitSlop={{ top: 8, bottom: 8, left: 4, right: 4 }}>
                              <View style={{
                                width: dotI === index ? 16 : 6, height: 6, borderRadius: 3,
                                backgroundColor: dotI === index ? theme.accent : `${theme.subtext}40`,
                              }} />
                            </TouchableOpacity>
                          ))}
                        </View>
